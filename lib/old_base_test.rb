...
module MA1000AutomationTool
  class BaseTest < Test::Unit::TestCase
    ...
    #Reference static TD manager from test method
    def td_proc
      BaseTest::require_td_manager if !@@td_proc
      @@td_proc
    end
    ...
    class << self
      ...
      def startup(test_type = TestType::UNKNOWN, sec_test_type = TestType::UNKNOWN)
        ...
        #Initialize test_ref as arbitrary class to support assertions
          ...
          #Reset retry flag
            ...
            #Make testability driver connection if required
            if test_type == TestType::TDRIVER
              if sec_test_type == TestType::UNKNOWN
                raise "Secondary test type not specified in startup method while calling super(TestType::TDRIVER, TestType = <?>) in script file '#{get_script_name}'!"
              else
              test_type = sec_test_type
              end

              #Open testability driver manager
              require_td_manager
            end
            ...
            #Assign current cmd_proc to TD manager
            @@td_proc.cmd_proc = @@cmd_proc if @@td_proc
      ...
      #Initialize testability driver manager
      def require_td_manager
        if !@@td_proc || @@td_proc.connected? == false

          require 'tdriver'

          #Connect to QTTAS running on device
          $test_logger.log("\nConnecting to QTTAS server at SUT_ID '#{FWConfig::SUT_ID}' (#{$comm_ip_address}) and APP '#{FWConfig::GUI_APP_NAME}'...", true)
          @@td_proc = TDManager.new(FWConfig::SUT_ID, FWConfig::GUI_APP_NAME, $comm_ip_address)

          $test_logger.log("Connected to GUI App via QTTAS server.", true)
        else
          $test_logger.log("Connection to GUI App via QTTAS server is already established!")
        end
      end
    end
    ...
    def ensure_conn
      ...
      if (@@current_test_type != TestType::SAMPLE) && (@@current_test_type != TestType::TDRIVER)
      ...
      @@td_proc.ensure_device_status if @@td_proc
      @@cmd_proc.ensure_device_status

      #Reset secondary connection, if exists
      @@sec_cmd_proc.reset_connection(true) if @@sec_cmd_proc
      elsif @@current_test_type == TestType::TDRIVER
        #Raise exception if no connection
        raise "No connection establised with Testability Driver!" if !@@td_proc

      #Ensure terminal is up and running
      @@td_proc.ensure_device_status
      @@cmd_proc.ensure_device_status
      end
    ...
    def exe_in_loop(time_in_sec = 10, &block)
      ...
      loop do
        ...
        #Terminate and wait till simu finger and flash card threads are gracefully exited
          ...
          err_msg = "#RECONNECTED#\n" + err_msg if @@cmd_proc.is_reconnected || (@@td_proc && @@td_proc.is_reconnected)
